...
describe('Notification routes integration', () => {
  ...
  it('GET /api/v1/notifications/counts returns counts map', async () => {
    const counts = { '123': 2 };
  ...
  it('POST /api/v1/notifications/:conversationId/read marks as read and returns updated count', async () => {
  ...
  it('POST /api/v1/notifications/:conversationId/read returns 401 when not authenticated', async () => {
...
describe('Notification controller unit tests (integrated)', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  it('getUserNotifications returns notifications via res.json on success', async () => {
    const fake = [{ id: 1 }];
  ...
  it('getUserNotifications forwards errors to next', async () => {
  ...
  it('getUnreadCounts returns counts via res.json on success', async () => {
    const counts = { 'c1': 2 };
    mockRepo.getUnreadCountByConversation.mockResolvedValueOnce(counts);
    const req = { user: { id: 7 } };
    const res = { json: jest.fn() };
  ...
  it('getUnreadCounts forwards errors to next', async () => {
  ...
  it('markAsRead calls repository and returns updated count', async () => {
